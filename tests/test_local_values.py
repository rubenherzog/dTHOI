...
def test_empirical_local_entropy_mean_matches_global_dense_and_sparse():
    ...
    for mode in ("dense","sparse"):
        provider=CountingEntropyProvider(_data(), estimator="empirical", count_mode=mode)
        entropy, local=provider.entropy_with_local(subsets)
        torch.testing.assert_close(local[0].mean(dim=1), entropy[:,0], rtol=0.0, atol=1e-12)


def test_miller_madow_local_entropy_mean_matches_corrected_global():
    provider=CountingEntropyProvider(_data(), estimator="miller_madow", count_mode="sparse")
...
def test_all_local_measures_average_to_global_values():
    ...
    values, local=output
    torch.testing.assert_close(local[0].mean(dim=1), values[:,0,:], rtol=0.0, atol=1e-12)
    torch.testing.assert_close(local[0][...,2], local[0][...,0]-local[0][...,1], rtol=0.0, atol=1e-12)
    torch.testing.assert_close(local[0][...,3], local[0][...,0]+local[0][...,1], rtol=0.0, atol=1e-12)


def test_dense_and_sparse_local_measures_match():
    subsets=torch.tensor([[0,1,2],[1,2,3]])
    dense=nplets_measures(_data(),subsets,count_mode="dense",local_values=True,max_local_values_per_batch=7)
    sparse=nplets_measures(_data(),subsets,count_mode="sparse",local_values=True,max_local_values_per_batch=7)
...
def test_unequal_dataset_lengths_remain_separate():
    first=_data()
    second=_data()[:4]
    result=dthoi.information_measures([first,second],[[0,1,2]],local_values=True,max_local_values_per_batch=4)
    assert result.local is not None
    assert result.local.values[0].shape==(1,7,4)
    assert result.local.values[1].shape==(1,4,4)
    torch.testing.assert_close(result.local.values[0].mean(dim=1),result.values[:,0,:],rtol=0.0,atol=1e-12)
    torch.testing.assert_close(result.local.values[1].mean(dim=1),result.values[:,1,:],rtol=0.0,atol=1e-12)


def test_public_api_does_not_compute_locals_by_default_and_exposes_named_local_views():
    ...
    torch.testing.assert_close(result.local.o_information[0],result.local.values[0][...,2])
